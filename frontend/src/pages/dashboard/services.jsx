...
const Services = () => {
  ...
  console.log(`loged in user ${user._id}`);
  useEffect(() => {
    const fetchServices = async () => {
      console.log("Fucntion is called");
      console.log(`inside fetch the user is ${user?._id}`);
      ...
      try {
        ...
        // Replace with the actual way to get the mentor's ID
  ...
  const handleEditService = async (values) => {
    setLoading(true);
    try {
      console.log("service is try for edit", editingService._id);

      const response = await service.editService(editingService._id, values);

      // Edit service through API call
      ...
      console.error("Error editingggg service:", error);
